...
def save_img(opts, model, loader, device):
    ...
    with torch.no_grad():
      for i, (images, labels) in tqdm(enumerate(loader)):
        ...
        Image.fromarray(pred).save('results/%d_pred_.png' % i)
...
def main():
    ...
    best_score = 0.0
    cur_itrs = 0
    cur_epochs = 15 # for continue training, interrupt in middle
    ...
    interval_loss = 0
    mIoU_per_epoch = torch.load('miou_list.pt')['miou']
    while True:  # cur_itrs < opts.total_itrs:
        ...
        torch.save(miou_save,'miou_list.pt')
        ...
        plt.suptitle(f'mIOU curve\n {owner}: {timestamp_str}', fontsize=16)
        # plt.title('mIOU curve')
        plt.xlabel('Epochs')
        plt.ylabel('mIOU')
        plt.grid(True)
        plt.savefig("mIoU_curve_"+str(cur_epochs)+".png")
