...
from utils import loss as my_loss
import torch
...
def get_argparser():
    ...
    parser.add_argument("--ckpt", default="checkpoints/best_deeplabv3plus_resnet50_VOC_os17.pth", type=str,
...
def save_img(opts, model, loader, device):
    ...
    with torch.no_grad():
      for i, (images, labels) in tqdm(enumerate(loader)):
        ...
        Image.fromarray(image).save('results/%d_image.png' % i)
        Image.fromarray(target).save('results/%d_target.png' % i)
        Image.fromarray(pred).save('results/%d_pred.png' % i)
...
def main():
    ...
    os.environ['CUDA_VISIBLE_DEVICES'] = '0' # use 0 works for my env
    ...
    # learning_rate=0.01 and momentum=0.9
    # backbone is 0.1x the main learning rate.
    ...
    # raise NotImplementedError
    # step learning rate schedular that reduce the learning rate by 0.9x every 1,000 iterations.
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer,step_size=1000,gamma=(1-0.9))# Set up learning rate scheduler
    ...
    # raise NotImplementedError
    # !!! use ignore_index=255 or there are errors!
    criterion = my_loss.MyLoss(device)
    ...
    best_score = 0.0
    cur_itrs = 0
    cur_epochs = 21 # for continue training, interrupt in middle
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        ...
        if opts.continue_training:
            ...
            print("best score with itr: "+str(best_score)+", "+str(cur_itrs))
    ...
    # save result
    save_img(opts=opts,model=model,loader=train_loader,device=device)
    # return
    # ==========   Train Loop   ==========#
    ...
    interval_loss = 0
    mIoU_per_epoch = torch.load('miou_list2.pt')['miou']
    while True:  # cur_itrs < opts.total_itrs:
        ...
        torch.save(miou_save,'miou_list2.pt')

        current_datetime = datetime.datetime.now()
        timestamp_str = current_datetime.strftime('%Y-%m-%d')
        owner ='chuangm'

        x_size=range(1,len(mIoU_per_epoch)+1)
        plt.clf()
        plt.plot(x_size,mIoU_per_epoch)
        plt.suptitle(f'mIOU curve with new loss\n {owner}: {timestamp_str}', fontsize=16)
        # plt.title('mIOU curve')
        plt.xlabel('Epochs')
        plt.ylabel('mIOU')
        plt.grid(True)
        plt.savefig("mIoU_curve_new_"+str(cur_epochs)+".png")
