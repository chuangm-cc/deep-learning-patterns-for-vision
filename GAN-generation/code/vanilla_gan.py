...
def training_loop(train_dataloader, opts):
    ...
    for _ in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            # torch.nn.BCELoss()
            D_real_loss = torch.nn.functional.binary_cross_entropy_with_logits(D_real_output, torch.ones_like(D_real_output)) # loss
            ...
            # torch.nn.BCELoss() not working here!
            D_fake_loss = torch.nn.functional.binary_cross_entropy_with_logits(D_fake_output, torch.zeros_like(D_fake_output))
            ...
            D_fake_output = D(fake_images)
            # G_loss = torch.nn.BCELoss()(D_fake_output, torch.ones_like(D_fake_output))
            G_loss = torch.nn.functional.binary_cross_entropy_with_logits(D_fake_output, torch.ones_like(D_fake_output))
            ...
            D_total_loss_list.append(D_total_loss.item())
            G_loss_list.append(G_loss.item())
            iteration_list.append(iteration)

    fig, axs = plt.subplots(2, 2, figsize=(10, 10))
    axs[0, 0].plot(iteration_list, D_fake_loss_list)
    axs[0, 0].set_title('D_fake_loss')

    axs[0, 1].plot(iteration_list, D_real_loss_list)
    axs[0, 1].set_title('D_real_loss')
    axs[1, 0].plot(iteration_list, D_total_loss_list)
    axs[1, 0].set_title('D_total_loss')
    axs[1, 1].plot(iteration_list, G_loss_list)
    axs[1, 1].set_title('G_loss')
    ...
    fig.suptitle(f'Loss plots\n {owner}: {timestamp_str}', fontsize=16)

    plt.savefig('lists_plot.png')
...
def create_parser():
    ...
    parser.add_argument('--data_preprocess', type=str, default='advanced') #'basic'
