...
class DCGenerator(nn.Module):
    ...
    def __init__(self, noise_size, conv_dim=64):
        ...
        # make this same
        conv_dim = 32
        #  first layer (up conv1) it is better to directly apply convolution
        # layer without any upsampling to get 4x4 output.
        self.up_conv1 = nn.Sequential(
          # padding can't be small:
          #  Kernel size: (4 x 4). Kernel size can't be greater than actual input size
          # paddimg to 3:get 4*4
          nn.Conv2d(noise_size, conv_dim*8, 4, padding=3),
          nn.InstanceNorm2d(conv_dim*8),
          nn.ReLU()
        )
        # use 'same' in padding to avoid: Kernel size can't be greater than actual input size
        self.resnet2 = ResnetBlock(conv_dim*8, norm='instance', activ='relu')
        self.up_conv2 = up_conv(conv_dim*8, conv_dim*4, 3, padding='same', norm='instance', activ='relu')

        self.resnet3 = ResnetBlock(conv_dim*4, norm='instance', activ='relu')
        self.up_conv3 = up_conv(conv_dim*4, conv_dim*2, 3, padding='same', norm='instance', activ='relu')

        self.resnet4 = ResnetBlock(conv_dim*2, norm='instance', activ='relu')
        self.up_conv4 = up_conv(conv_dim*2, conv_dim, 3, padding='same', norm='instance', activ='relu')

        self.resnet5 = ResnetBlock(conv_dim, norm='instance', activ='relu')
        self.up_conv5 = up_conv(conv_dim, 3, 3, padding='same', norm='no', activ='tanh')
...
class DCDiscriminator(nn.Module):
    ...
    def __init__(self, conv_dim=64, norm='instance'):
        ...
        self.resnet2 = ResnetBlock(32, norm, activ='relu')
        self.conv2 = conv(32, 32*2, 4, 2, 1, norm, False, 'relu')

        self.resnet3 = ResnetBlock(32*2, norm, activ='relu')
        self.conv3 = conv(32*2, 32*4, 4, 2, 1, norm, False, 'relu')

        self.resnet4 = ResnetBlock(32*4, norm, activ='relu')
        self.conv4 = conv(32*4, 32*8, 4, 2, 1, norm, False, 'relu')

        # self.resnet5 = ResnetBlock(32*8, norm, activ='relu')
        self.conv5 = conv(32*8, 1, 4, 1, 0, 'no', False)
